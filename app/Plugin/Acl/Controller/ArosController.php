...
class ArosController extends AclAppController {
	...
	function admin_check($run = null) {
		...
		$preAro = ClassRegistry::init('Aro');
		foreach ($roles as $role) {
			/*
			 * Check if ARO for role exist
			 */

			$aro = $preAro->find('first', array('conditions' => array('model' => $role_model_name, 'foreign_key' => $role[$role_model_name][$this->_get_role_primary_key_name()])));
	...
	function admin_users() {
		...
		$preAro = ClassRegistry::init('Aro');
		foreach ($users as &$user) {
			$aro = $preAro->find('first', array('conditions' => array('model' => $user_model_name, 'foreign_key' => $user[$user_model_name][$this->_get_user_primary_key_name()])));
	...
	function admin_role_permissions() {
		...
		foreach ($actions as $full_action) {
			...
			if ($controller_name != 'App') {
				foreach ($roles as $role) {
					$aro_node = ClassRegistry::init('Aro')->node($role);
					if (!empty($aro_node)) {
						$aco_node = ClassRegistry::init('Aco')->node('controllers/' . $full_action);
						if (!empty($aco_node)) {
	...
	function admin_user_permissions($user_id = null) {
		...
		if (empty($user_id)) {
			 ...
			 */
			$user_aro = ClassRegistry::init('Aro')->node($user);
			if (empty($user_aro)) {
	...
	function admin_clear_user_specific_permissions($user_id) {
		 ...
		 * Check if the user exists in the ARO table
		 */
		$node = ClassRegistry::init('Aro')->node($user);
		if (empty($node)) {
	...
	function admin_grant_all_controllers($role_id) {
		$role = ClassRegistry::init(Configure :: read('acl.aro.role.model')); //& $this->{Configure :: read('acl.aro.role.model')};
		$role->id = $role_id;

		/*
		 * Check if the Role exists in the ARO table
		 */
		$node = ClassRegistry::init('Aro')->node($role);
		if (empty($node)) {
	...
	function admin_get_role_controller_permission($role_id) {
		...
		$aro_node = ClassRegistry::init('Aro')->node($role_data);
		if (!empty($aro_node)) {
			...
			foreach ($controller_actions as $action_name) {
				...
				$aco_node = ClassRegistry::init('Aco')->node('controllers/' . $aco_path);
				if (!empty($aco_node)) {
	...
	function admin_grant_role_permission($role_id) {
		//$role = ClassRegistry::init(Configure :: read('acl.aro.role.model')); //ClassRegistry::init(Configure :: read('acl.aro.role.model')); //& $this->{Configure :: read('acl.aro.role.model')};
		$role = ClassRegistry::init('Group');
		$role->id = $role_id;
	...
	function admin_get_user_controller_permission($user_id) {
		...
		$aro_node = ClassRegistry::init('Aro')->node($user_data);
		if (!empty($aro_node)) {
			$plugin_name = isset($this->params['named']['plugin']) ? $this->params['named']['plugin'] : '';
			$controller_name = $this->params['named']['controller'];
			$controller_actions = $this->AclReflector->get_controller_actions($controller_name);
	...
	function admin_grant_user_permission($user_id) {
		 ...
		 * Check if the user exists in the ARO table
		 */
		$aro_node = ClassRegistry::init('Aro')->node($user);
		if (!empty($aro_node)) {
			$aco_node = ClassRegistry::init('Aco')->node('controllers/' . $aco_path);
			if (!empty($aco_node)) {
				if (!$this->AclManager->save_permission($aro_node, $aco_path, 'grant')) {
