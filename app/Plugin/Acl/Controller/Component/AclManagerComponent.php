...
//		}
	...
	public function save_permission($aro_nodes, $aco_path, $permission_type) {
		if (isset($aro_nodes[0])) {
			...
			if ($specific_permission_needed) {
				if ($permission_type == 'allow' || $permission_type == 'grant') {
					$this->Session->write('var1', $aro_model_data);
					$this->Session->write('var2', $aco_path);
					//$this->Session->write('var3', $this->Acl);
				...
				$aco_node = ClassRegistry::init('Aco')->node($aco_path);
				if (!empty($aco_node)) {
					$aco_id = $aco_node[0]['Aco']['id'];

					$specific_permission = ClassRegistry::init('Aro')->Permission->find('first', array('conditions' => array('aro_id' => $aro_id, 'aco_id' => $aco_id)));

					if (!empty($specific_permission)) {
						if (ClassRegistry::init('Aro')->Permission->delete(array('Permission.id' => $specific_permission['Permission']['id']))) {
							return true;
						} else {
	...
	private function get_first_parent_permission_right($aro_node, $aco_path) {
		...
		while (strpos($aco_path, '/') !== false && !isset($parent_permission_right)) {
			...
			$parent_aco_node = ClassRegistry::init('Aco')->node($aco_path);
			if (!empty($parent_aco_node)) {
				$parent_aco_id = $parent_aco_node[0]['Aco']['id'];

				$parent_permission = ClassRegistry::init('Aro')->Permission->find('first', array('conditions' => array('aro_id' => $aro_id, 'aco_id' => $parent_aco_id)));
